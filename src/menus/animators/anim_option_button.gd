...
func _hover() -> void:
	...
	self.scale = Vector2(1.7,0.5)
	_reset_tween()
	_tween = get_tree().create_tween()
	_tween.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	_tween.tween_property(self,"scale",Vector2(1.2,1.2),0.5).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_BACK)
...
func _pressed() -> void:
	...
	_tween.tween_property(self,"scale",Vector2(1.12,0.125),0.05)
	_tween.tween_property(self,"scale",Vector2(1.2,1.2),1.15).set_trans(Tween.TRANS_ELASTIC)
